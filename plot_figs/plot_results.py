...
def save_boxplots(paths,labels,obs_data,experiment_label,fig_save_dir,xlabel='Model',ylabel='Pearson R',title='',fig_width=11,gene_set=None):
    """
    ...
    # get gene sets 
    if gene_set is None: 
        gene_set = np.load(f'{paths[0]}gene_list.npy')
    ...
    SAGEnet.plot.sb_boxplot(comb_res,xlabel=xlabel,ylabel=ylabel,save_dir=save_dir,save_name='per_gene_train_gene',title=f'{title} train gene per gene Pearson R \n n={len(comb_res)}',save_fig=True,dot_size=5,dot_alpha=.1,fig_width=fig_width)
    ...
    for i in range(len(paths)):
        ...
        comb_res[labels[i]] = corr
    SAGEnet.plot.sb_boxplot(comb_res,xlabel=xlabel,ylabel=ylabel,save_dir=save_dir,save_name='per_gene_test_gene',title=f'{title} test gene per gene Pearson R \n n={len(comb_res)}',save_fig=True,dot_size=5,dot_alpha=.3,fig_width=fig_width)

    # plot across gene test gene 
    comb_res = pd.DataFrame(index=list(range(1)))
    for i in range(len(paths)): 
        if type(labels[i])==str and 'non contrastive' in labels[i]:
            pred = pd.read_csv(f'{paths[i]}diff_predictions.csv',index_col=0).loc[test_genes].mean(axis=1).values
        else: 
    ...
    SAGEnet.plot.sb_boxplot(comb_res,xlabel=xlabel,ylabel=ylabel,save_dir=save_dir,save_name='across_gene_test_gene',title=f'{title} test gene across gene Pearson R',save_fig=True,dot_size=10,dot_alpha=1,fig_width=fig_width,boxplot=False)
