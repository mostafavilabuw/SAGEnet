...
For a notebook tutorials, see `example_notebooks/`. `GEUVADIS_example_psagenet.ipynb` is a fully runnable notebook that goes through PersonalGenomeDataset creation and p-SAGE-net model training with the publicly available GEUVADIS dataset [[2]](#2), using expression data preprocessed by Rastogi et al. [[3]](#3). `GEUVADIS_example_psagenet.ipynb` does the same but for a ReferenceGenomeDataset and r-SAGE-net model. `example_usage.ipynb` uses toy data provided in this repo to describe the inputs required to create your own datset. It details important parameters to change to make SAGEnet fit your needs, and walks through the process we use to train models on the ROSMAP dataset.  
